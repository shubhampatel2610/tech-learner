...
This closes the loop back to the start of this app: modules 1-14 are the *material*; this six-step method, practiced repeatedly, is what turns that material into a reliable skill.`,
  ...
  quiz: [
    {
      id: 'gr-q1',
      type: 'mcq',
      prompt: 'You just wrote a correct, working solution. What should you do next, according to the golden rules?',
      options: ['Immediately move to the next problem', 'State its complexity and consider whether it can be optimized', 'Rewrite it in a different language for practice', 'Nothing - correct is correct'],
      answerIndex: 1,
      explanation: 'Step 5 (Optimize) comes right after correctness is established - stating complexity and reasoning about improvements is expected, not optional polish.',
    },
    {
      id: 'gr-q2',
      type: 'boolean',
      prompt: 'Dry-running a solution on paper before coding it is mainly useful for beginners, and experienced engineers can safely skip it.',
      answer: false,
      explanation: 'Dry-running catches logic errors cheaply regardless of experience level - skipping it under time pressure is a common cause of avoidable bugs even for experienced engineers, not just a beginner\'s crutch.',
    },
    {
      id: 'gr-q3',
      type: 'mcq',
      prompt: 'Testing only the example given in the problem statement is a failure of which step?',
      options: ['Understand', 'Dry Run', 'Test', 'Optimize'],
      answerIndex: 2,
      explanation: 'Step 4 (Test) specifically requires deliberately trying edge cases (empty, single-element, extremes) beyond the happy-path example already given.',
    },
    {
      id: 'gr-q4',
      type: 'fill',
      prompt: 'Fill in the six golden rules in order: Understand, Dry Run, Code, Test, ___, Repeat.',
      answer: 'Optimize',
      explanation: 'The full sequence is Understand -> Dry Run -> Code -> Test -> Optimize -> Repeat.',
    },
  ],
  ...
  examples: [
    {
      level: 'Beginner',
      title: 'Two Sum, walked through all six steps',
      problem: 'Given an array of integers and a target, return the indices of the two numbers that add up to the target.',
      approach:
        "UNDERSTAND: exactly one solution exists, indices returned in any order, can't use the same element twice. DRY RUN: trace [2,7,11,15], target=9 by hand - track a 'seen' map of value -> index; at 7, check if 9-7=2 was seen (yes, at index 0) - found it. CODE: translate that exact trace into a single loop with a hash map. TEST: single-element array (no valid answer, but problem guarantees a solution so this may be out of scope - confirm during Understand), negative numbers, duplicate values summing to target. OPTIMIZE: this IS the optimized O(n) version - a nested-loop brute force would be O(n²); mention that tradeoff explicitly. REPEAT: label this 'hash map complement lookup' for next time.",
    ...
    {
      level: 'Intermediate',
      title: 'Valid Parentheses, walked through all six steps',
      problem: 'Determine whether a string of brackets ()[]{} is validly nested and matched.',
      approach:
        "UNDERSTAND: only these six characters appear (confirm with the interviewer if unstated); empty string counts as valid. DRY RUN: trace '([)]' by hand with a stack - push '(', push '[', see ')' but top is '[' - mismatch, invalid. Trace '()[]{}' too - a valid case - to confirm the happy path also works. CODE: translate directly - push openers, on a closer check the stack top matches. TEST: empty string (valid, stack stays empty), a single unmatched closer ')' (stack is empty when you need to pop - guard this), all openers with no closers (stack non-empty at the end - guard this too). OPTIMIZE: already O(n) time, O(n) space - the theoretical minimum, since you must read every character once. REPEAT: label this 'stack for nested/matching structure'.",
      complexity: 'O(n) time, O(n) space',
      code: [
        {
          language: 'python',
    ...
    {
      level: 'Advanced',
      title: 'Course Schedule, walked through all six steps',
      problem: 'Given course prerequisites as directed edges, determine if all courses can be completed (i.e. no cyclic dependency).',
      approach:
        "UNDERSTAND: prerequisites form a directed graph; 'can finish all courses' means the graph has no cycle - restating it this way is what makes the algorithm obvious. DRY RUN: trace Kahn's algorithm by hand on a small 4-course example with a cycle vs one without, tracking in-degrees and a processing queue, BEFORE writing code (see the Graphs module's dry run for this exact trace). CODE: translate the traced Kahn's algorithm - compute in-degrees, queue up in-degree-0 nodes, process and decrement neighbors. TEST: no prerequisites at all (trivially finishable), a self-loop (course requiring itself - immediate cycle), a diamond dependency (A needs B and C, both need D - not a cycle, must not be flagged as one). OPTIMIZE: already O(V+E), the minimum needed to even read the input graph. REPEAT: label this 'topological sort via Kahn's algorithm, cycle detection for free'.",
